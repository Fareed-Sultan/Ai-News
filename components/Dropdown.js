  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        ...
        aria-expanded={open}
        aria-label={label}
        className={`flex shrink-0 items-center gap-1.5 bg-slate-100/90 dark:bg-white/[0.08] border rounded-full pl-3 pr-2.5 py-1 text-xs font-semibold text-slate-800 dark:text-slate-200 transition-all ${
          open
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`absolute z-50 mt-2 w-60 bg-white dark:bg-[#0D1527] rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl origin-top flex flex-col overflow-hidden ${
              align === "right" ? "right-0" : "left-0"
            }`}
          >
            {showSearch && (
              <div className="p-2.5 border-b border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-black/20">
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-white/[0.06] border border-slate-200 dark:border-white/10 rounded-xl text-xs">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent outline-none text-slate-800 dark:text-slate-100 placeholder:text-slate-400 text-xs font-medium"
                    autoFocus
                  />
                  {query && (
                    <button
                    ...
                    >
                      ...
                      >
                        <span className="flex items-center gap-2 truncate">
                          ...
                          {item.meta && (
                            <span className="text-rose-600 text-[10px] font-extrabold bg-rose-100 dark:bg-rose-500/20 px-2 py-0.5 rounded-full shrink-0">
                              {item.meta}
                            </span>
                          )}
                        </span>
